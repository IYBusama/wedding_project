    ...
    @include('components.admin_sidebar')
        ...
        <main class="flex-1 p-8 overflow-auto">
            ...
            <section class="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto">
                <form class="space-y-4" action="{{route('AdminUser.editUser',$user->id)}}" method="post">
                    @method('PUT')
                    @csrf
                    <!-- User Name Input -->
                    <div>
                        <label for="UserName" class="block text-gray-700 font-semibold mb-1">User Name</label>
                        <input type="text" id="UserName" name="name" class="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{old('name',$user->name)}}">
                    </div>
                    
                    <!-- Email Input -->
                    <div>
                        <label for="UserEmail" class="block text-gray-700 font-semibold mb-1">Email</label>
                        <input type="email" id="UserEmail" name="email" class="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{old('email',$user->email)}}">
                    </div>
